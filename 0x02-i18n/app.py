...
def get_timezone() -> Time:
    ...
    try:
        if time_zone and pytz.timezone(timezone):
            return pytz.timezone(time_zone)
        elif g.user and pytz.timezone(g.user['timezone']):
...
def index() -> str:
    ...
    c_time = format_date(datetime.now(), locale=get_locale())

    return render_template('index.html', login_user=g.user, current_time=c_time)
